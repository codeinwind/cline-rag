...
3. Get access to the API with the following code:
    ...
    ```ts
    const clineExtension = vscode.extensions.getExtension<ClineAPI>("budianwang.cline-rag")
    ...
    **Note:** To ensure that the `budianwang.cline-rag` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

    ```json
    "extensionDependencies": [
        "budianwang.cline-rag"
    ]
    ```
